...
Take-home assignment from EBANX, meant to stay as simple as possible: an in-memory account API built with NestJS + TypeScript, no database, no persistence.
...
- **Validation**: plain `typeof` guards per field (`assertString`/`assertPositiveNumber`), dispatched by event type in the controller. Tried DTO classes with `class-validator`/`class-transformer` first, but three request shapes with 2-3 fields each didn't justify the decorator/reflection machinery — dropped the dependency for the same runtime safety with less indirection.
